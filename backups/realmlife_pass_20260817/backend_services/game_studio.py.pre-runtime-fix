        ...
        if r == "quiz_adventure":
            ...
            if st.get("mode") == "side_scroll":
                if int(st.get("schema_version") or 1) >= 2:
                    # XY Engine V2 — true 2D multi-elevation schema
                    if not (st.get("solids") or []):
                        errs.append(f"stage {i+1}: schema v2 level needs solids[]")
                    if not isinstance(st.get("spawn"), dict) or "x" not in (st.get("spawn") or {}):
                        errs.append(f"stage {i+1}: schema v2 level needs spawn {{x,y}}")
                    if not (st.get("world_w") and st.get("world_h")):
                        errs.append(f"stage {i+1}: schema v2 level needs world_w and world_h")
                    if not (st.get("portals") or []):
                        errs.append(f"stage {i+1}: schema v2 level needs at least one portal")
                    if not (st.get("enemies") or []):
                        errs.append(f"stage {i+1}: needs enemies")
                    for p in (st.get("portals") or []):
                        rk = p.get("required_key_id")
                        if rk and not any(rk in {k2.get("key_id") for k2 in (s2.get("keys") or [])}
                                          for s2 in stages[:i + 1]):
                            errs.append(f"stage {i+1}: portal '{p.get('portal_id')}' requires key "
                                        f"'{rk}' which is never placed in this or an earlier stage")
                    continue
