...
export default function LifeSimRuntime({ game, progress, onExit }) {
  ...
  useEffect(() => {
    ...
    // ========================================================
    // REALMLIFE BED STUCK SAVE RESCUE V1
    //
    // Same philosophy as the mountain/off-map rescue:
    // if an old save loads inside the bed zone, immediately
    // place the resident on the open center floor.
    // ========================================================
    ...
    if (
      ...
      console.warn(
        "[RealmLife] Bed-stuck save rescued to open floor."
      );
    }
    ...
    const resolveRealmLifeAvatar =
      async () => {
        try {
          ...
          // REALMLIFE INDEPENDENT AVATAR — REAL GLB STARTERS
          if (
            nexusAvatar?.mode
            === "starter"
          ) {
            ...
            /*
             * Historical "starter" selections safely become
             * Starter 1. Starter 2 remains Starter 2.
             */
            ...
            if (
              ...
              throw new Error(
                `RealmLife starter model missing: ${selectedStarter}`
              );
            }

            console.info(
              "[RealmLife] Real starter GLB",
              selectedStarter,
              starterCfg.modelUrl
            );

            /*
             * Walking / Running are expected to come from the
             * animations EMBEDDED in the uploaded starter GLB.
             *
             * RealmLife universal interaction motions remain
             * available as optional motions. createLifeAvatar()
             * already protects avatar loading if an optional
             * external motion fails.
             */
            return createLifeAvatar({
              ...
              /*
               * Starter 1 / Starter 2 contain real embedded
               * Walking + Running skeletal animations.
               * Drive their mixer independently.
               */
              selfDriveMixer:
                true,

              /*
               * Both verified Meshy starter GLBs contain one
               * active skin with all 24 animated joints below
               * the node named Armature.
               */
              animationRootName:
                "Armature",
            });
      ...
      if (residentAvatar) {
        ...
        if (sequence) {
          ...
          try {
            ...
            // ==================================================
            // REALMLIFE SLEEP EXIT RECOVERY V1
            //
            // Never leave the player root inside the bed collider.
            // Sleep always finishes beside the bed with normal
            // movement/pathing restored.
            // ==================================================
            ...
            if (
              ...
              // REALMLIFE SLEEP SHOWER RESCUE V1
              //
              // Temporarily wake completely away from the bed
              // so we can verify the bed collider independently.

              // REALMLIFE FINAL BED WAKE POSITION V1
              //
              // Wake at the foot of the bed, safely outside
              // the bed geometry. The bed itself no longer
              ...
              if (
                Number.isFinite(wakeX)
                &&
                Number.isFinite(wakeZ)
              ) {
              ...
              // Return completely to floor/standing orientation.
              resident.position.y =
                0;
              ...
              // Kill any stale route or interaction command that
              // could immediately pull the player back into bed.
              moveTargetRef.current =
                null;
              ...
              console.info(
                "[RealmLife] Sleep finished — player released beside bed."
              );
            }
