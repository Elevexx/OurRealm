...
## Cover Generator + Widget Polish + Guest Conversion (Aug 5, 2026) ✅
...
## Aug 17 2026 — REALMLIFE FULL HOUSING/PRIVACY/MOBILE/PERF PASS (iteration 153 — all green)
- Safety backup: /app/backups/realmlife_pass_20260817/ (frontend lifesim + backend services + games.py + landing).
- P0 ownership fix: per-user home. Backend housing_status now returns home_anchor {founder, lot_seq, x, z, w, d} + property.city_id/city_lot_seq. Frontend LifeSimRuntime fetches home_anchor BEFORE world build; entire starter interior (walls, bed/shower/toilet/fridge/stove/sofa/TV/computer + new decor: dining set, rug, plants) builds at the player's OWN lot via HOME_X/HOME_Z offset. Founder stealth keeps origin estate; origin gets "Founder Residence" shell + footprint collider for non-founders. Old origin saves auto-migrate to own lot. Player-placed furniture now stored HOME-RELATIVE. CUTAWAY targets own shell at own lot (buildNeighborhoodWorld(scene,{ownLot})).
- P0 joystick fix: first-person NORMAL-mode camRight basis sign-corrected at source (firstRightSign); removed keyboard-only mx special case → keyboard AND joystick now compose identically in all 8 directions; REVERSE untouched.
- Property privacy: other homes sealed shells; INTERACT near another front door → POST access-check → allowed: unlock collider; denied: auto entry-request ("🔒 Private residence — entry request sent"). Fixed pre-existing 500 (ObjectId) in request_property_entry (insert_one(dict(doc))).
- Home beacons (original RealmLife design): GET /api/games/{id}/realmlife/world/beacons (no identity leakage); lifeSimHomeBeacons.js — cyan additive beam + floating octahedron, own = brighter/bigger, distance cull 380/640, near-LOD animation, 60s refresh.
- World upgrade: instanced backyards behind all 100 homes (lawn+3 fence rails+2 bushes = 3 draw calls) + ~106 instanced street/plaza trees (2 draw calls) in lifeSimNeighborhood.js.
- Mobile UX: Needs collapsed to icon by default (realmlife-needs-toggle, red when any need <50, expand/minimize); ROTATE FOR FULL VIEW popup (Nexus-pattern, sessionStorage realmlife_rotate_dismissed, fullscreen+orientation.lock on ROTATE); safe-area env() insets for joystick/JUMP/INTERACT/top bar; container min(620px,100dvh)/maxHeight 100dvh (page already fixed inset-0 layer).
- Performance: createAdaptiveDPR governor in lifeSimGraphics.js (fps<34 → -0.25 DPR down to 0.75; fps>56 → restore to tier cap; cap follows graphics tier via onChange).
- data-testids added: realmlife-home-btn, realmlife-cutaway-btn, realmlife-property-btn, realmlife-pov-btn, realmlife-control-direction-btn, realmlife-menu-btn, realmlife-jump-btn (testing agent action item).
- Household surrender refund already existed and verified untouched: 50% of each contributor's recorded contribution, idempotent via refunded_50 flag (services/realmlife_property.py _refund_contributors).
...
## Aug 17 2026 — PASS 2: PROPERTY AUTHORITY + GUEST INTERIORS + STREETS + CHAT + SKIN RECOLOR + LOAD SPEED (iteration 154 all green)
- Backup: /app/backups/realmlife_pass2_20260818/
- Property authority: access-check returns is_household_member/guest_access_mode/level_access; POST /realmlife/property/guest-access (public/private/custom + per-level); housing includes guest_interior_access. Fixed set_guest_interior_access arg bug (current -> current['id']).
- Guest furnished interiors: lifeSimGuestInterior.js streams host interior at lot on authorized entry; property-scoped guest CUTAWAY (grantGuestLot/revokeGuestLot in neighborhood privacy api); 30s watchdog revokes live (evicts player outside, re-adds collider); '🔒 THIS LEVEL IS LOCKED' when ground locked.
- First-entry: fresh/migrated users spawn inside THEIR house with CUTAWAY auto-enabled.
- In-world chat: presence carries chat{text,id} (chat_text/chat_id/chat_ts + server_ts in response); lifeSimChatBubble.js — auto-sized navy/cyan canvas sprites with trail dots, one bubble per user, fade+scale-out, 55m distance limit; Enter (desktop) / 💬 button (mobile) compact input; POV shows own msg via HUD line. Verified visually.
- Streets: asphalt rows + sidewalks + lane markings + instanced driveways + instanced street lamps; yards shrunk 8.5->4.2 deep (no road overlap); trees moved off street band. Shell quality pass: fascia/overhang, door lintel, chimney on all 100 homes.
- Avatar load: gameplay GLBs optimized 61MB->2.4MB / 55MB->2.2MB (gltf-transform draco+webp1024, *_game.glb, uploaded via storage adapter); REALMLIFE_PLAYER_MODELS points to _game.glb; cachedLoadGLTF (memory + CacheStorage 'realmlife-models-v1') + preloadRealmLifeModel; landing progressive hero (2.4MB stand-in -> 4K swap).
- Skin recolor (P3): GPU shader mask via onBeforeCompile in realmLifeRecolor.js (sRGB color-distance to ref #f2a67c, luminance-preserving pow 0.6). ROOT CAUSE of earlier failures: setHeroLoaded(slot) same-value bailout meant no render followed async install — recolor now applied inside install paths via customSkinRef. 14 skin tones in catalog (realmlife_players.py SKIN_TONES). HAIR/EYE recolor NOT possible on current GLBs (single baked material; hair shares black with clothing — python mask experiments proved contamination): hair/eye + clothing/accessory controls HIDDEN for Player 1/2. Needs re-exported GLBs with separated materials.
- Mobile POV/WORLD toggle in top HUD left of HOME (realmlife-mobile-pov-btn).
- Property panel: GUEST INTERIOR ACCESS section (public/private/custom + ground/second/third toggles); hook setGuestAccess.
- Testing: iteration_154.json — backend 10/10 (two-owner, unauthorized-denial, invite->guest->level-access->evict cycle, chat presence, truncation, optimized assets), frontend 100% (chat, landing gating, mobile HUD, panel persistence). Production build passed. NOT deployed.
- NEXT PASS QUEUED (user message 3, NOT yet implemented): canonical multiplayer property state (wall/floor colors, furniture instances replicated to observers), ADD LEVEL/BASEMENT for 5000 FP, furniture library + edit mode (move/rotate/duplicate/remove/color), Stealth-house observer parity, per-level construction. Skin range (item 1) DONE; hair/eye (item 2) blocked on GLB re-export.

## Aug 17 2026 — HOUSE LEVELS + EXTERIOR LEVEL SYNC (slice 1 of Property State Sync)
- Backend: POST /realmlife/property/add-level {direction: above|below} — 🔥5,000 FP atomic burn (fire_wallets find_one_and_update, founder free), caps 3 above/3 below, persists levels_above/levels_below on property; fire_wallet_transactions type realmlife_level_burn. housing property payload + beacons now include levels_above/levels_below.
- Frontend: privacy api setLotLevels(lotSeq, above, below) rebuilds any lot's shell to the persisted level count (ownShell is now `let`); runtime applies canonical levels from beacons on load + 60s refresh → ALL observers see the same exterior height. Panel: HOUSE LEVELS section (GROUND ✓ / ADD — 🔥5,000 FIRE POWER / LOCKED chain), hook addHouseLevel.
- VERIFIED (curl, two accounts): burn 5400→400 exactly once, level persisted (above:2), insufficient FP rejected with FP message, stealth's beacons show lot2 at 2 levels. Panel screenshot verified (LEVEL 2 ✓ BUILT, LEVEL 3 ADD, BASEMENT chain). Production build passed.
- STILL QUEUED (directive 3): interior flex-rooms + stairs for new levels, furniture library + move/rotate/duplicate/color/remove editor, wall/floor color editing, full furniture/interior replication to observers, Player 1/2 GLB re-export for hair/eye color (needs separated materials — Meshy re-export, requires user's Meshy key).

## Aug 19 2026 — GENESIS CITY V1 FINAL PASS (fork, lean mode)
- Furnished all 7 remaining storefronts in lifeSimCityDistrict.js via compact furnishGenesisVenue(): plaza-restaurant (chef_special), fresh-grocery (browse_goods), night-lounge (dance_floor), river-grill (waterside_dine), pulse-club/Club 178 (club_dance), central-offices (creator_desk), river-hotel (lobby_lounge). Each: graphite feature wall + neon header + sign label + signature genesisInteractive + decor + sit-seating + exterior door neon posts.
- City Outfitters 9/9: fitting pod (view_look), style hologram (browse_styles), exterior neon marquee + label.
- Citywide neon/cyber pass: Main St/plaza/riverwalk emissive edge glow strips; ground darkened (plaza/sidewalks/riverwalk/bridges/crosswalk hex swaps); venue neon; residential shells get shared-material neon trim (cyan own / violet others) in lifeSimNeighborhood.js.
- Public hangout zones: Downtown Plaza fire ring (plaza_hangout) + 4 neon benches (sit); Riverwalk overlook (river_overlook) + 2 benches.
- LifeSimRuntime ACTION_EFFECTS: 9 new FREE actions (chef_special, browse_goods, dance_floor, waterside_dine, club_dance, creator_desk, lobby_lounge, plaza_hangout, river_overlook).
- Business mgmt: all 10 venues already registered in realmlife_business.py (claim/release/card verified present) — no changes needed.
- Verified: webpack dev compile clean; in-world screenshots (plaza/venues render w/ neon, Sunrise Café + Outfitters marquee visible); production build passed 68.1s. NOT deployed (user deploys).
- NOTE: pending model-swap task from Aug 17 fork remains UNSTARTED (4 uploaded skin-variant GLBs in /data/ourrealm/models; _game.glb optimization jobs were launched in /tmp/opt_*.log — check completion before wiring).
