...
RUNTIMES = [
    # Status truth (founder directive): "live" requires a published demo passing the
...
async def make_quote(body: dict, current: CurrentUser):
    ...
    if not idea or not rt or rt[4] not in ("live", "beta"):
        raise HTTPException(status_code=400, detail="Pick a Live or Beta runtime, a style and describe your game")
