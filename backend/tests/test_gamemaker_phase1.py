...
class TestCatalog:
    def test_founder_sees_catalog(self, founder):
        ...
        for planned in ("open_world_rpg", "shooter"):
            rt = by_id.get(planned)
            assert rt is not None, f"missing runtime {planned}: keys={list(by_id.keys())}"
            assert rt.get("status") == "planned", f"{planned} status={rt.get('status')}"
