...
# ============================================================
# REALMLIFE V7A HOME DESTINATION
#
# Returns the authoritative exterior spawn directly in front
# of the user's RealmLife residence.
#
# Normal residents:
# physical spawn comes from their assigned city_lot_seq.
#
# Stealth Founder:
# current Founder showcase residence remains at the existing
# RealmLife origin property until the dedicated per-city
# Founder estate registry is completed.
# ============================================================


REALMLIFE_CITY_COLUMNS = [
    -130,
    -104,
    -78,
    -52,
...
def _realm_lot_geometry(
    ...
    x = REALMLIFE_CITY_COLUMNS[
        col_index
    ]


    z = REALMLIFE_CITY_ROWS[
        row_index
    ]
...
async def home_destination(
    game_id,
    current,
):
    # Ensure the user's housing/city assignment exists first.
    location = await resolve_player_city(
        game_id,
        current,
    )
    ...
    if not prop:
        raise HTTPException(
            status_code=409,
            detail=(
                "RealmLife home could not be resolved."
            ),
        )
    ...
    if _realmlife_is_founder(
        ...
        # Current Spanish residential shells face toward +Z.
        # Place the resident outside the private property line
        # directly in front of the assigned house.
        spawn = {
            "x":
                geometry[
                    "x"
    ...
    # Keep multiplayer presence logically in the same city.
    await (
        db.realmlife_world_presence
        .update_one(
            {
