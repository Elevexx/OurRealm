...
async def migration_dry_run() -> dict:
    ...
    missing, have, likes_to_convert, no_class, samples = 0, 0, 0, 0, []
    dupes = []
...
async def migration_execute(founder: dict) -> dict:
    ...
    created, skipped, likes_converted, failures = 0, 0, 0, []
    now = _now_iso()
    async for t in db.tracks.find({}, {"_id": 0}):
        try:
            ...
            doc = _post_doc_for_track(t, user, source_composer="migration",
                                      ...
                                      migration_source="sound_backfill")
            await db.posts.insert_one(doc)
            created += 1
            # Track hearts → 1× Fire on the canonical post (idempotent,
            # zero pool consumption — same rules as the Like→Fire migration).
    ...
    report = {"mode": "execute", "posts_created": created, "skipped_existing": skipped,
