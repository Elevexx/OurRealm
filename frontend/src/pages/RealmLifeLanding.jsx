...
export default function RealmLifeLanding() {
  ...
  useEffect(() => {
    ...
    if (!scene || !previewReady || custom === null) return;
    if (modelRef.current) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col md:flex-row" style={{ background: "#060d16", color: "#eef7fb" }} data-testid="realmlife-landing">
      ...
      <div className="w-full md:w-[430px] md:max-h-none max-h-[58vh] overflow-y-auto p-4 md:p-5" style={{ background: "rgba(9,18,30,.96)", borderLeft: "1px solid rgba(34,211,238,.18)" }}>
        ...
        {tab === "PLAYER" && (
          <div>
            <div className="text-[10px] font-black opacity-70 mb-2">CHOOSE YOUR SILHOUETTE — BOTH FREE</div>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {["style_a", "style_b"].map((s2) => (
                <button
                  key={s2}
                  type="button"
                  data-testid={`realmlife-style-${s2}`}
                  onClick={() => setStyle(s2)}
                  className="rounded-xl p-3 text-left"
                  style={{
                    background: style === s2 ? "rgba(34,211,238,.16)" : "rgba(255,255,255,.05)",
                    border: style === s2 ? "1px solid rgba(34,211,238,.6)" : "1px solid rgba(255,255,255,.12)",
                  }}
                >
                  <div className="font-black text-sm">{s2 === "style_a" ? "STYLE A" : "STYLE B"}</div>
                  <div className="text-[10px] opacity-65 mt-1">
                    {s2 === "style_a" ? "Broad frame · athletic" : "Slim frame · agile"}
                  </div>
                  <div className="text-[10px] mt-1.5" style={{ color: "#22d3ee" }}>FREE STARTER</div>
                </button>
              ))}
            </div>
            <div className="text-[10px] opacity-60 mb-3">
              Starter outfit: white shirt · brown shorts · white shoes. Fully customizable.
            </div>
          </div>
        )}
        ...
        {tab === "AVATARS" && catalog && (
          <div>
            <button type="button" data-testid="realmlife-avatar-starter"
              onClick={() => selectAvatar("starter")}
              className="w-full rounded-xl p-3 mb-2 text-left"
              style={{
                background: selected === "starter" ? "rgba(34,211,238,.16)" : "rgba(255,255,255,.05)",
                border: selected === "starter" ? "1px solid rgba(34,211,238,.6)" : "1px solid rgba(255,255,255,.12)",
              }}>
              <div className="font-black text-sm">STARTER {selected === "starter" && "· SELECTED"}</div>
              <div className="text-[10px] opacity-65">Your customized RealmLife player</div>
            </button>
            ...
            {state?.is_founder && (
              <button type="button" data-testid="realmlife-avatar-founder"
                ...
                <div className="font-black text-sm" style={{ color: "#ffcf99" }}>
                  ⚡ FOUNDER STEALTH {selected === "founder_stealth" && "· SELECTED"}
                </div>
                <div className="text-[10px] opacity-70">Exclusive Founder body — permanently yours</div>
              </button>
            )}

            <div className="text-[10px] font-black opacity-70 mt-3 mb-1.5">PREMIUM REALMLIFE AVATARS</div>
            {catalog.avatar_tiers.map((t2, i) => {
              const owned = unlocks.has(t2.id);
              return (
                <div key={t2.id} className="rounded-xl p-3 mb-2" style={{ background: "rgba(255,255,255,.045)", border: `1px solid ${selected === t2.id ? t2.accent : "rgba(255,255,255,.1)"}` }}>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-black text-sm" style={{ color: t2.accent }}>
                        {"◆".repeat(i + 1)} {t2.name.toUpperCase()} {selected === t2.id && "· SELECTED"}
                      </div>
                      {!owned && (
                        <div className="text-[10px] opacity-70 mt-0.5">
                          FIRE POWER REQUIRED: 🔥{t2.fire_power_required.toLocaleString()}
                        </div>
                      )}
                    </div>
                    {owned ? (
                      <button type="button" data-testid={`realmlife-tier-select-${t2.id}`}
                        ...
                        {selected === t2.id ? "SELECTED" : "SELECT"}
                      </button>
                    ) : (
                      <button type="button" data-testid={`realmlife-tier-unlock-${t2.id}`}
                        ...
                        className="px-2.5 py-1 rounded-lg text-[10px] font-black"
                        style={{ background: "rgba(255,120,73,.16)", border: "1px solid rgba(255,120,73,.45)", color: "#ffd9c8" }}>
                        BURN 🔥{t2.fire_power_required.toLocaleString()}
                      </button>
                    )}
                  </div>
                </div>
            ...
            <div className="text-[10px] opacity-50 mt-1">
              Premium hero bodies are being crafted — tier trim glows on your player now; full hero models arrive shortly.
            </div>
          </div>
        )}
