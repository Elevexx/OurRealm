...
export default function NexusWorld({ mode = "play", world, zoneId = "plaza", username = "you",
  onSelect, selectedId, onEntityMove, onPortal, refreshKey = 0 }) {
  ...
  useEffect(() => {
    ...
    zone.entities.forEach((e) => {
      ...
      if (e.type === "box" || e.type === "pillar" || e.type === "ramp") {
        ...
        m = makeAvatar(e.color || "#e8c07a", e.props?.label || "NPC");
    ...
    const player = makeAvatar("#37c8ff", mode === "play" ? username : null);
    scene.add(player);
    ...
    if (mode === "play") {
      apiClient.get("/nexus/position").then((r) => {
        ...
        if (p && p.zone_id === zone.id) { player.position.set(p.x, Math.max(0, p.y), p.z); yaw = p.ry || 0; }
      }).catch(() => {});
        ...
        apiClient.post("/nexus/presence", {
          ...
          (r.data.players || []).forEach((pl) => {
            ...
            if (!remotes[pl.user_id]) {
              remotes[pl.user_id] = { grp: makeAvatar("#ff9a5c", pl.username), tgt: new THREE.Vector3(pl.x, pl.y, pl.z), ry: pl.ry };
          ...
          Object.keys(remotes).forEach((id) => {
            if (!seen.has(id)) { scene.remove(remotes[id].grp); delete remotes[id]; }
          });
        }).catch(() => {});
      }, 300);
